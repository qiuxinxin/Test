...
IPL and_template_original_imgs(IPL src1,IPL src2)
{
    ...
    data=(uchar *)img->imageData;//imagedata是指向存储图像像素值数组的指针,内容是这个数组的首地址
    ...
    for(i=0;i<height;i++){
        for(j=0;j<width;j++)
        {
            for(k=0;k<channels;k++)
            {
                ...
                {
                    data3[i*step3+j*channels3+k]=data[i*step+j*channels+k];//依次读取B、G、R3个通道中的数据,判断原二值图像src2中白色部分（即目标藻类部分），将原图像src1的目标藻类部分数据赋给最终的目标图像dstimg，背景则默认黑色。
                }
            }
        }
    }
